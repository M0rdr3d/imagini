...
db.connect((err) => {
    ...
    console.log("db initializing data...");
    db.query(
    ...
    app.post("/uploads/:name", bodyparser.raw({
        ...
        db.query("INSERT INTO images SET?",{
            ...
            if(err){
                return res.send({status : "error", code: err.code, errmsg: err.message});
            }
    ...
    app.get("/stats", (req,res)=>{
        db.query("SELECT COUNT(*) total" +
         ", SUM(size) size " +
         ", MAX(date_created) last_created FROM images",
         (error, rows)=>{
             if(err){
                 res.status(500).end();
             }
             rows[0].uptime = process.uptime();
             return res.send(rows[0]).status(666).end();
         });
    });
